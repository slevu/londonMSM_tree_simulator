...
# head(sa)
.thr <- melt(a[2:5],  measure.vars = c(6, 7))
# head(thr)
...
p <- ggplot(tot, aes(x = factor(age), y = value, fill = variable), outlier.colour= alpha("black", 0.1)) + geom_boxplot() + theme(legend.position="none", legend.title = element_blank(), strip.text.x = element_text(size = 12)) 
...
p %+% tot_win[tot_win$L1 %in% c('NA','0.015'),] + facet_wrap(~ variable + L1, nrow = 1, scales = "free_y", labeller=labeller(variable = c(outdegree = "Outdegree", size = "Cluster size", nbhsize = "Neighborhood \n size")))
