...
Great ! Distributions look similar for rate threshold = 0.015 subs/site vs time threshold = 5 years (Sergei Pond did say that somewhere ?. Need to check the input of mutation rate in branch conversion)
```{r distribution cluster size}
```

Plot cluster sizes from both methods
```{r scatter, fig.path = './figure/'}
```
Correlation is `r corcoef`

Now, to plot base boxplot of cluster size vs covariates from tMRCA clustering and neighborhood method, load all previous results
```{r load hivclust}
```
Re-arrange in one dataframe
```{r arrange}
```
Plot
```{r mrca boxplot for all covariates, fig.path = './figure/'}
```

And a scatter plot matrix between the 3 clustering methods + SA
```{r scatter plot matrix, fig.path = './figure/'}
```

And a scatter bubble matrix (note out-degrees are in log10) 
```{r scatter bubble matrix, fig.path = './figure/'}
```
