...
THRESHOLD_W <- .01 
cd4s <- read.csv('ukdrd_data/cd4s.csv' )
dem <- read.csv('ukdrd_data/demographics.csv')

## load cluster sids 
require(ape)
tre <- read.tree( '../ukdrd_data/clusterForBEAST0.interleaved_phyml_tree.txt')
cl_sids <- sapply(strsplit( tre$tip.label 
 , '_'), function(sp) sp[1] )
Wfns <- list.files( path= '../infProbs20/', full.names=T)
# combine
n <- length(cl_sids)
